...
export function ValueEditor({ dataType, value, onChange, className }: ValueEditorProps) {
  ...
  switch (dataType) {
    ...
    case 'null':
      return (
        <div className={cn('p-4 bg-muted rounded-md text-center text-muted-foreground', className)}>
          null
        </div>
      );

    case 'date': {
      const dateValue = deserializedValue ? new Date(deserializedValue as string | number | Date) : new Date();
      return (
        <Popover>
    ...
    case 'regexp': {
      const regexpValue = (deserializedValue as { pattern: string; flags: string }) || { pattern: '', flags: '' };
      return (
          ...
          <div className="grid grid-cols-3 gap-2">
            <div className="col-span-2">
              ...
              <Input
                ...
                onChange={(e) =>
                  handleChange({ ...regexpValue, pattern: e.target.value })
                }
                placeholder="正则表达式模式..."
                className="font-mono"
              />
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) =>
                  handleChange({ ...regexpValue, flags: e.target.value })
                }
                placeholder="gim"
                className="font-mono"
                maxLength={5}
