      ...
      try {
        ...
        const formattedActivities = logs.map((log) => {
          ...
          return {
            ...
            time,
            type:
              log.action.includes('delete') || log.action.includes('reset') ? 'warning' : 'success',
          };
        });
      ...
      >
        ...
        >
          <div>
            ...
            <p className="mt-1.5 text-muted-foreground">这是您的管理控制台概览，数据实时更新。</p>
          </div>
          <motion.div
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {statCards.map((stat, index) => (
            <StatCard key={stat.title} {...stat} loading={stats.loading} delay={index * 0.1} />
          ))}
        </div>
