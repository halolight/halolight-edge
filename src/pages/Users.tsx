...
export default function Users() {
  ...
  const handleAddUser = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/api-gateway/api/create-user`,
        {
          method: 'POST',
          headers: {
  ...
  const columns: Column<UserWithRole>[] = [
    ...
    },
    {
      ...
      render: (_, user) => (
        <span
          className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium ${roleConfig[user.role || 'user'].color}`}
        >
        ...
        >
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                此操作无法撤销。确定要删除用户 "{selectedUser?.full_name || selectedUser?.email}"
                吗？
              </DialogDescription>
            </DialogHeader>
            <DialogFooter>
