...
│   ├── api-gateway/        # API 网关 (Swagger UI + 代理 + 用户创建)
│   │   └── index.ts
│   └── system-info/        # 系统信息查询
│       └── index.ts
...
supabase functions deploy system-info
...
### api-gateway
**功能**：API 网关 - Swagger UI、Supabase 代理、用户创建、Token 验证

**端点**：`/functions/v1/api-gateway`

**子路由**：
- `/` - Swagger UI 文档
- `/api/create-user` - 创建新用户 (需要 Admin 权限)
- `/api/env` - 环境变量配置检查
- `/api/token/verify` - API Token 验证
- `/auth/*` - Supabase Auth 代理
- `/rest/*` - Supabase REST 代理

**请求示例**：
```bash
# 创建用户
curl -X POST \
  'https://your-project.supabase.co/functions/v1/api-gateway/api/create-user' \
